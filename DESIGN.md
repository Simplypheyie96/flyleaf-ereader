...
| **Selection menu** | **The app owns the selection once it settles** (`SPEC.md` § 6.1): the range is repainted through `::highlight(flyleaf-selection)` in the `--hl-select` wash, the browser's own range is cleared so the platform draws no callout, and the app-drawn menu positions from the range's own rects, 14px clear of the line and the handle's knob. Highlight (five tints) · Note · Copy · In this book · Find. One row, no wrap; a second row appears rather than shrinking the first. Hidden while a handle is held. |
| **Selection handles** **NEW** | Press has no selectable text, so no token. A 2px bar of `--stock-ink` the height of the line at each end of the range, with a 10px round knob of the same ink — **above** the start, **below** the end, the convention every phone taught. The bar and knob are the visible part; the thing the finger hits is an invisible 44px square around the knob. Drag moves that end to the word under the finger, snapped to word boundaries. No shadow, no halo: the same rule as everything else on the page. |
