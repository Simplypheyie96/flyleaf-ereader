...
export const makeBook = async file => {
    ...
    if (file.isDirectory) {
        ...
        // FLYLEAF PATCH 1: comic-book.js is not vendored (CBZ/CBR are out of
        // scope). The type is still detected, so a reader who opens one gets the
        // same clean UnsupportedTypeError as any other unsupported file rather
        // than the EPUB parser failing obscurely on a zip full of JPEGs.
        if (isCBZ(file)) {
            throw new UnsupportedTypeError('Comic book archives are not supported')
        }
        else if (isFBZ(file)) {
    ...
    // FLYLEAF PATCH 2: pdf.js is not vendored. PDFs are handled by our own
    // pdfjs-dist view and never reach makeBook; upstream's adapter would have
    // dragged in the 13MB vendored pdfjs. Detected and refused explicitly so a
    // PDF that somehow arrives here fails loudly instead of being mis-parsed.
    else if (await isPDF(file)) {
        throw new UnsupportedTypeError('PDF is handled by the PDF view, not makeBook')
    }
    else {
...
export class View extends HTMLElement {
    ...
    // FLYLEAF PATCH 3: initTTS() removed. tts.js is not vendored — speech is not
    // a feature of this reader. Removed rather than left in place because a
    // dynamic import that is never called still emits its own chunk, and
    // Workbox precaches every chunk in dist: the cost of the dead branch is
    // paid by every reader's install, not just by the one who calls it.
    startMediaOverlay() {
