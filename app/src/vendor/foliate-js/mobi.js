...
class MOBI6 {
    ...
    // FLYLEAF PATCH 6: KF8 has one of these and MOBI6 does not, so a book in
    // the older format had no content hook at all. Same name, same shape.
    ...
    async loadSection(section) {
        ...
        // FLYLEAF PATCH 6: the same `data` event `loadResourceBlob` already
        // dispatches for resources, dispatched for the SECTION document too.
        const detail = { data: result, type: this.#type }
...
class KF8 {
    ...
    async loadSection(section) {
        ...
        // FLYLEAF PATCH 6: as in MOBI6.loadSection above — `loadResourceBlob`
        // dispatches `data` for resources, this does it for the section.
        const detail = { data: this.serializer.serializeToString(doc), type: this.#type }
        const event = new CustomEvent('data', { detail })
        this.transformTarget.dispatchEvent(event)
        const newData = await event.detail.data
        const newType = await event.detail.type
